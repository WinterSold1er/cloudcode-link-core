...
export function calculateNetUsage(meta?: StreamUsageMetadata): any | null {
  if (!meta) return null
...
/**
 * Re-assembles incremental StreamChunk events back into structured blocks and message.
 */
export class BlockAssembler {
  private _blocks: any[] = []
  private _activeBlocks = new Map<number, any>()
  private _usage: any = null
  private _finish: any = null

  push(chunk: any): void {
    if (!chunk || typeof chunk !== 'object') return

    if (chunk.type === 'block-start') {
      const idx = chunk.index ?? this._blocks.length
      const block: any = {
      ...
      this._activeBlocks.set(idx, block)
    } else if (chunk.type === 'text-delta') {
      const b = this._activeBlocks.get(chunk.index)
      if (b) b.text += chunk.text ?? ''
    } else if (chunk.type === 'reasoning-delta') {
      const b = this._activeBlocks.get(chunk.index)
      if (b) b.text += chunk.text ?? ''
    } else if (chunk.type === 'tool-call-delta') {
      let b = this._activeBlocks.get(chunk.index)
      if (!b) {
        b = { type: 'tool-call', name: chunk.name ?? '', arguments: '', id: chunk.id }
        this._activeBlocks.set(chunk.index, b)
      }
      ...
      for (const [idx, b] of this._activeBlocks.entries()) {
        this._blocks.push(b)
        this._activeBlocks.delete(idx)
      }
    }
  }
