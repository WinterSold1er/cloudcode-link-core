...
import { PROVIDER_ID, formatDuration, type CoreConfig } from './types/config-types.ts'
import { modelFamilyOf, type ManagedAccount, type ModelFamily } from './types/pool-types.ts'
...
export interface AgyAdapterDeps {
  ...
  /** Reads image bytes from storage. */
...
export class AgyAdapter {
  ...
  async listModels(provider = 'antigravity'): Promise<readonly LlmModelInfo[]> {
    ...
    try {
      ...
      while (attempt < maxAttempts) {
        ...
        if (this.deps.pool && account) {
          releaseAccount = await this.deps.pool.acquireAccount(account.id, 3)
        }
        ...
        try {
          ...
          for await (const chunk of mapSseStreamToChunks(res, options.signal, () => {
            ...
            if (chunk.type === 'finish') {
              const finishReason = (chunk as { reason?: { kind?: string } }).reason
