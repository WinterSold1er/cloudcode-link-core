// Direct CloudCode oneshot text runner.
import { isAbsolute, resolve } from 'node:path'
import { readFile } from 'node:fs/promises'
import type { CoreConfig } from './types/config-types.ts'
import { modelFamilyOf } from './types/pool-types.ts'
...
export async function runAgyOnce(
  ...
  const cfg = deps.cfg()
  const timeoutMs = req.timeoutMs ?? cfg.timeoutMs ?? 600_000
  ...
  try {
    ...
    const endpoints = deps.endpointCandidates || (cfg.endpointCandidates ? [...cfg.endpointCandidates] : undefined)
    ...
    for await (const chunk of mapSseStreamToChunks(streamResult.response, abortController.signal)) {
      if (chunk.type === 'text-delta') {
        ...
        const finishReason = (chunk as { reason?: { kind?: string; failure?: { message?: string } } }).reason
